...
//  mimamoriGPS
//
//  AWS REST API専用データサービス（Firebase・公共交通DB完全削除版）
//
...
class DataService: ObservableObject {
    ...
    @Published var currentDeviceLocation: DeviceLocation?  // 地図表示用
    @Published var currentTemperature: Temperature?
    @Published var isLoading: Bool = false
    @Published var errorMessage: String?
    @Published var locationHistory: [DeviceLocation] = []  // DeviceLocationに変更
    ...
    private func fetchCurrentData() async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                // 地図表示用にDeviceLocationに変換
                if let location = device.lastLocation {
                    self.currentDeviceLocation = DeviceLocation(from: location, deviceId: deviceId)
                }
    ...
    func fetchHistory(
        ...
        do {
            ...
            await MainActor.run {
                // HistoryEntryからDeviceLocationに変換（位置情報のみ）
                self.locationHistory = response.history.compactMap { historyEntry in
                    DeviceLocation(from: historyEntry, deviceId: deviceId)
                }
                print("✅ 履歴取得成功: \(response.count)件（位置情報: \(self.locationHistory.count)件）")
            }
